...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier paidEnough(uint256 _price) { require(msg.value >= _price, "not paid enough"); _;}
   ...
   */
  modifier forSale (uint256 _sku) {require(items[_sku].state == State.ForSale && items[_sku].price != 0, "item not for sale"); _;}
  modifier sold (uint256 _sku) {require(items[_sku].state == State.Sold, "item not sold"); _;}
  modifier shipped (uint256 _sku) {require(items[_sku].state == State.Shipped, "item not shipped"); _;}
  modifier received (uint256 _sku) {require(items[_sku].state == State.Received, "item not received"); _;}
  ...
  function fetchItem(uint256 _sku) public view returns (string memory name, uint256 sku, uint256 price, uint256 state, address seller, address buyer) {
